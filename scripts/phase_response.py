...
omega = fftfreq(N_SAMPLES, 1/SAMPLE_RATE)
print("fft omega:",omega.max())
...
w,h = signal.freqz(num, den, SAMPLE_RATE)
...
omega_axis = ind_mag#10*0.5*ind_mag/(np.pi)#10*(0.5 / (np.pi)) * ind_mag

ax[0].semilogx(omega_axis, (20*np.log10(np.abs(mag_resp))), color='r',label='Calculated Response')
