...
plt.figure()
plt.scatter(omega_single_out[0:max_index],magnitude_response_mean[0:max_index])
plt.show()
plt.figure()
plt.scatter(omega_single_out[nonzero_ind],magnitude_response_mean[nonzero_ind])
plt.show()
plt.figure()
for i in range(N_SWEEPS):
...
print(np.polynomial(fit,omega_x))
plt.figure()
plt.plot(fit)
plt.show()
