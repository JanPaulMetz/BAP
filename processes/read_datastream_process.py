...
def extract_packages_thread_target(n_packages,data_to_process_tx, start_data_extraction, data_bin, data_lock,new_data):
    ...
    data_to_process_queue = queue.Queue(maxsize=n_bytes)
    data_synced_queue = queue.Queue(maxsize=n_bytes//2)
    while True:
        ...
        with data_lock:
            ...
            # Queue
            data_to_process_queue.put(data_to_process[0:n_bytes])
            # Clear
            new_data.clear()
            # data_bin.clear()

        # Extract databin starting with ID byte
   
        data_to_process_bits = BitArray(data_to_process)
        bytes_to_process = data_to_process_queue.get()
        data_to_process_tx.send(bytes_to_process)

        print("BYTES ON PIPE", bytes_to_process, len(bytes_to_process))
        print("BYTES", bytes_to_process, len(bytes_to_process))
        print("first 2 byte", bytes_to_process[0:2], len(bytes_to_process[0:2]))
        # sync_status, data_synced = sync_on_id(data_to_process_bits, n_bytes)

            # id_found = False
            # while not id_found:
            #     print("FINDING ID")
            #     # get two bytes from queue, could use qsize method but is not reliable
            #     bytes_to_process = data_to_process_queue.get()
            #     # second_byte = data_to_process_queue.get()
            #     print("BYTES", bytes_to_process, len(bytes_to_process))
            #     print("first 2 byte", bytes_to_process[0:2], len(bytes_to_process[0:2]))

            #     header = (int(first_byte) >> 6) & 0b11
            #     print("HEADER", header)
            #     if header == 0b01:
            #         data_synced_queue.put(first_byte)
            #         # data_synced_queue.put(second_byte)
            #         data_synced_queue.put_nowait(data_to_process_queue.get())
            #         sync_status = True
            #         id_found = True

            # while not data_synced_queue.empty():
            #     data_to_process_tx.send(data_synced_queue.get())
